...
class Inception_ResNetv2(nn.Module):
    def __init__(self, in_channels=3, classes=9, k=256, l=256, m=384, n=384):
        ...
        self.linear2 = nn.Linear(768,192)
        self.linear3 = nn.Linear(192,96)
        self.linear4 = nn.Linear(96,36)
        self.linear5 = nn.Linear(36,classes)
        self.relu= nn.ReLU()
    def forward(self, x):
        x = self.features(x)
        x = self.conv(x)
        ...
        x = x.view(x.size(0), -1)
        x = self.linear5(self.relu(self.linear4(self.relu(self.linear3(self.relu(self.linear2(self.relu(self.linear1((x))))))))))
        return x